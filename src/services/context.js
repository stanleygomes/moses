...
export async function ensureDefaultContextFiles() {
  ...
  try {
    ...
    const defaultBasePrompt = await fs.readFile(
      path.join(PROMPTS_DIR, "base-prompt.md"),
      "utf-8",
    );
  ...
  try {
    ...
    const defaultPractices = await fs.readFile(
      path.join(PROMPTS_DIR, "mr-practices.md"),
      "utf-8",
    );
